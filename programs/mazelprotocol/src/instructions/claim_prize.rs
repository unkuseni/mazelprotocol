...
//! It handles:
//! - Verification that the draw is complete
...
//! - User stats updates
//! - Free ticket issuance for Match 2
...
/// 2. Verifies the draw has been finalized
/// 3. Calculates the match count against winning numbers
/// 4. Looks up the prize amount from draw result
/// 5. Transfers USDC from prize pool to player (or issues free ticket for Match 2)
/// 6. Updates ticket state with match count and prize
/// 7. Updates user statistics
///
/// # Prize Tiers
/// - Match 6: Jackpot (variable, from draw result)
/// - Match 5: $4,000 (fixed) or rolldown amount
/// - Match 4: $150 (fixed) or rolldown amount
/// - Match 3: $5 (fixed) or rolldown amount
/// - Match 2: Free ticket ($2.50 value, handled separately)
/// - Match 0-1: No prize
///
/// # Arguments
...
pub fn handler(ctx: Context<ClaimPrize>) -> Result<()> {
    ...
    let draw_total_tickets = ctx.accounts.draw_result.total_tickets;
    let match_6_prize = ctx.accounts.draw_result.match_6_prize_per_winner;
    ...
    // Verify the draw has been finalized (prize amounts are set)
    require!(
        draw_total_tickets > 0 || match_6_prize > 0,
    ...
    let mut free_ticket_issued = false;

    if match_count == 2 && prize_amount > 0 {
        // Match 2 = Free ticket
        free_ticket_issued = true;
        msg!("Free ticket awarded for Match 2!");
    ...
    if prize_amount > 0 && match_count != 2 {
    ...
    if has_prize {
        ...
        msg!("  Free ticket: {}", free_ticket_issued);
    } else {
