...
7. [Jackpot LP Pool](#7-jackpot-lp-pool)
...
A house fee is collected on-chain on each draw. The fee rate scales with the current jackpot level and is configured on-chain by the program. This keeps ticket pricing simple while the fee adjusts automatically as the jackpot grows, and lower fees during rolldown events encourage ticket volume.

### 1.2 Smart Contract Implementation

The fee is selected by the program from on-chain config based on the current jackpot balance and rolldown state. See the program source / IDL for the current fee values.

```rust
/// Select the house fee (in basis points) from on-chain config.
pub fn calculate_house_fee_bps(
    jackpot_balance: u64,
    is_rolldown_pending: bool
) -> u16 {
    // Fee comes from the on-chain fee schedule (jackpot-level tiers).
    read_fee_from_config(jackpot_balance, is_rolldown_pending)
}
```

### UI/UX Considerations

The purchase UI surfaces the current house fee tier and the resulting prize-pool split so players can see how their ticket is allocated.
...
Using the exact pari-mutuel EV simplification: **EV_rolldown = J/N + $0.3659** (where J = jackpot, N = tickets sold).

```
Scenario: Jackpot at $2.0M, 700k tickets sold during rolldown
Probability of rolldown: 50%

Expected rolldown contribution (EV = J/N + $0.37):
├── If rolldown triggers (50% chance):
│   ├── Pari-mutuel EV = $2,000,000 / 700,000 + $0.37 = $3.23
│   └── Total EV during rolldown: $3.23
├── If no rolldown (50% chance):
│   └── Normal EV: $0.88

Weighted EV = (0.5 × $3.23) + (0.5 × $0.88) = $2.06
*EV = J/N + 0.3659 (pari-mutuel terms cancel; Match 2 free ticket is the constant)

As jackpot approaches hard cap (assuming 700k tickets):
├── At $2.25M: Probability = 100%, EV = $3.58 (+43%)
├── At $2.0M:  Probability = 50%,  Weighted EV = $2.06
├── At $1.875M: Probability = 25%, Weighted EV = $1.47

Players get increasing +EV as jackpot grows, with guaranteed +EV at hard cap.
Lower ticket volume = higher EV per ticket (pari-mutuel self-adjusts).
```
...
> **🔒 PRIZE TRANSITION SYSTEM:** All Quick Pick prizes START as FIXED amounts during normal mode, then TRANSITION to PARI-MUTUEL (shared pool) during rolldown events. This hybrid system ensures payout liability is ALWAYS CAPPED while maintaining attractive +EV windows for players.
...
├── Match 1: C(5,1) × C(30,4) / 324,632 = 137,025/324,632 = 42.2%
...
Like the main lottery, Quick Pick Express uses a house fee that scales with the jackpot level. The fee rate is configured on-chain by the program.
...
**Pari-Mutuel Transition Trigger:** If (Winner Count × Fixed Prize) > Prize Pool, automatic transition to pari-mutuel occurs to cap the total payout.

### 4.7 🔥 Prize Structure - Rolldown Mode — PARI-MUTUEL (FAVORABLE DISTRIBUTION)

> **🔒 CRITICAL TRANSITION:** During rolldown, ALL prizes transition from FIXED to **PARI-MUTUEL**. The total payout is CAPPED at exactly the jackpot amount ($30,000-$50,000), regardless of ticket volume or winner count.
...
|-------|------------|-------------|-------------|---------|----------------|
| **5** | 0% | $0 | $0 (no winner) | — | $0 |
| **4** | 60% | $18,000 | ~$3,247* | `Pool ÷ Winners` | $1.50 |
| **3** | 40% | $12,000 | ~$74.6* | `Pool ÷ Winners` | $1.00 |
| **Total EV** | | | | | **$2.50** |

*\*Estimated prizes at ~12,000 tickets. Actual = Pool ÷ Winner Count (pari-mutuel formula). More tickets = more winners = lower per-winner prize, but the total payout STAYS CAPPED at jackpot amount.*
...
**🔒 WHY PARI-MUTUEL CAPS THE PAYOUT:**
...
- The total payout is mathematically CAPPED
...
│                  Jackpot: $30,000                        │
│         Total Payout Liability: EXACTLY $30,000          │
└─────────────────────────────────────────────────────────┘
                           │
           ┌───────────────┴───────────────┐
           ▼                               ▼
    ...
    * Estimated at ~12,000 tickets. Actual = Pool ÷ Winners
    ...
    🔒 PAYOUT PROTECTION: Total payout = EXACTLY $30,000
...
Each ticket's prize pool is allocated as follows (percentages of the prize pool):
...
// Quick Pick Dynamic Fees — fee tiers are configured on-chain and read from
// config at runtime (see the program source / IDL for the current values).
...
| Match 4 | ~$3,247 × (1/2,164) | **$1.50** |
| Match 3 | ~$74.6 × (1/74.6) | **$1.00** |
| **Total** | | **$2.50** |
...
### 4.12 Cycle Economics — FIXED → PARI-MUTUEL

> **🔒 PRIZE MODE TRANSITION:** Normal mode uses FIXED prizes. Rolldown mode uses PARI-MUTUEL prizes. This hybrid system caps the total payout during high-volume rolldown events.

The prize pool is **self-sustaining**: ticket sales fund it, and prizes (both fixed and pari-mutuel) are paid out from it. During rolldown, prizes transition to pari-mutuel, so the total payout is capped at the jackpot amount regardless of ticket volume or winner count.

### 4.13 Why This Works: The Rolldown Favorable Distribution

```
┌─────────────────────────────────────────────────────────┐
│            QUICK PICK EXPRESS MECHANICS                 │
├─────────────────────────────────────────────────────────┤
│                                                         │
│  NORMAL MODE (11-14 draws per cycle):                   │
│  ├── Prizes start as FIXED amounts                      │
│  ├── Jackpot grows toward cap                           │
│  └── Pari-mutuel transition is armed                    │
│                                                         │
│  ROLLDOWN MODE (1 draw per cycle):                      │
│  ├── Player edge: +67%                                  │
│  ├── Full jackpot distributes to M4/M3 winners          │
│  ├── Total payout is capped at the jackpot              │
│  └── Players win — creates a favorable distribution event.                 │
│                                                         │
│  NET RESULT:                                            │
│  ├── Players have predictable +EV windows ✅            │
│  ├── Payouts remain capped by pari-mutuel ✅            │
...
| **Dynamic Fees** | ✅ Dynamic (configured on-chain) | ✅ Dynamic (configured on-chain) |
...
6. **Expected Profit**: ~$1.00 per ticket during rolldown (+67%)

#### Recommended Bankroll

| Risk Level | Bankroll | Tickets/Rolldown | Expected Profit |
|------------|----------|------------------|-----------------|
| Conservative | $150 | 100 | $100 |
| Moderate | $450 | 300 | $300 |
| Aggressive | $1,500 | 1,000 | $1,000 |

⚠️ **Variance Warning**: Even with +67% edge, individual draws can lose. Recommended minimum 10+ rolldown participations to realize expected value.
...
| Feature | KPI | Target |
|---------|-----|--------|
| Dynamic Fee | Fee tier read from on-chain config | Configured on-chain |
...
**Core Innovation:** Instead of the protocol alone funding the $500,000 jackpot seed, a community of Liquidity Providers (LPs) collectively provides the capital and earns a share of the fees collected on-chain from ticket sales.

### 7.2 Economic Model

A house fee is collected on-chain on each ticket; the LP pool participates in a share of that fee. The prize-pool portion of each ticket is allocated as follows (percentages of the prize pool):
...
**LP reward share** is configurable via admin and set on-chain.

### 7.3 Rewards

LP rewards accrue from the share of fees collected on-chain. The reward share is set on-chain by the program admin. See the program source / IDL for the current values.
...
1. Every ticket purchase routes a share of the house fee to the LP pool
...
| `set_lp_config(reward_bps)` | Authority | Set the LP reward share (configured on-chain) |
...
> The LP reward share is stored on-chain in the `LpPool` account (`lp_reward_bps`) and set via `set_lp_config`. Its value is configured by the program admin; see the program source / IDL for the current setting.
...
- `buy_ticket`: LP pool and LP pool USDC accounts added as optional accounts. When present and active, the house fee is split between the LP pool and the protocol treasury portion.
