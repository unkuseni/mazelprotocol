...
// Dynamic Fee — the house fee is configured on-chain and read from config at
// runtime (jackpot-level tiers). See the program source / IDL for the values.
...
pub const SOFT_CAP_ROLLDOWN_RATE_BPS: u16 = 3000; // DEPRECATED: replaced with probabilistic rolldown
...
// Quick Pick Dynamic Fee — fee tiers are configured on-chain and read from
...
pub struct LotteryState {
    ...
    /// Fixed prize balance (39.4% of the ticket pool, earmarked for Match 3/4/5)
...
pub struct QuickPickState {
    ...
    /// Current house fee (dynamic based on jackpot level)
    pub house_fee_bps: u16,
...
rate) are rejected. A bonded `challenge_draw` instruction lets anyone dispute a
finalized draw's counts: the challenger posts a `CHALLENGE_BOND` ($500 USDC,
escrowed in the insurance pool), their alternative counts + evidence hash are
recorded on-chain, and prize claims for **that draw only** are frozen (the rest
of the lottery keeps operating). The authority adjudicates via
`resolve_challenge` — upheld challenges are refunded the bond plus a
`CHALLENGE_REWARD` from the insurance pool; dismissed challenges are slashed to
the insurance pool. If the authority never resolves, anyone can call
`release_challenge` after `CHALLENGE_RESOLUTION_TIMEOUT` (7 days) for a neutral
outcome (bond refunded, no reward, claims unfrozen), so a draw's claims can
...
| House Fee | Configured on-chain (dynamic) | Varies by jackpot level |
| Prize Pool | Remainder after house fee | — |
...
> **🔒 PARI-MUTUEL PROTECTION:** All prizes START as FIXED amounts during normal operation, then TRANSITION to PARI-MUTUEL (shared pool) during rolldown events and high-volume draws. This hybrid system ensures **payout liability is ALWAYS CAPPED** while maintaining attractive +EV windows for players.
...
| Match Tier | Fixed Prize | Prize Mode | Payout Liability |
|------------|-------------|------------|-------------------|
...
**🔒 PARI-MUTUEL PROTECTION:** Total payout = EXACTLY the jackpot amount. Payout liability is mathematically CAPPED regardless of ticket volume or winner count.

#### Example: Why Pari-Mutuel Caps the Payout
...
**🔒 KEY INSIGHT:** With pari-mutuel, higher volume = lower per-winner prizes, but payout liability STAYS CAPPED. This is the fundamental protection that makes the protocol sustainable at any scale.

#### Implementation Benefits

1. **🔒 Payout Limitation**: Maximum payout = jackpot amount (CAPPED)
...
5. **📊 Predictable Economics**: The protocol can model worst-case scenarios precisely
...
| Mode | Match 4 Prize | Match 3 Prize | Payout Liability |
|------|---------------|---------------|-------------------|
