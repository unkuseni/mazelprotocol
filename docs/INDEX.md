...
## 🗄️ Data Storage Topology

MazelProtocol uses **three independent persistence layers** — they do not share
data and must not be confused with one another:

| Component | Store | Location | Purpose |
|-----------|-------|----------|---------|
| `app/` | Cloudflare **D1** (SQLite) | binding `CHAT_DB` | Syndicate chat messages + reactions |
| `jebu/` | **libSQL / Turso** (`@libsql/client`) | `file:jebu.db` or `libsql://…` | Off-chain provably-fair lottery state |
| `bot-rust/` | Local **JSON** files | `data/` | Draw bot stats & pause state |
| `programs/` | **Solana on-chain** accounts | PDAs | Canonical source of truth for the lottery |

> The on-chain programs (`programs/`) are the **source of truth**. `jebu/` and
> the Rust bots hold off-chain mirrors / operational state and must reconcile
> against the chain. `app/`'s D1 chat DB is unrelated to lottery state.
...
> **🔒 CRITICAL DESIGN FEATURE:** All prizes START as FIXED amounts during normal operation, then TRANSITION to PARI-MUTUEL (shared pool) during rolldown events and high-volume draws. This hybrid system ensures **payout liability is ALWAYS CAPPED** while maintaining attractive +EV windows for players.
...
| Dynamic Fee Range | Configured on-chain (dynamic) | — | ADVANCED_FEATURES.md |
...
A house fee is collected on-chain on each draw. The fee schedule scales with the jackpot level and is configured on-chain by the program. See [ADVANCED_FEATURES.md](./ADVANCED_FEATURES.md) for the mechanism.
...
*\*Rolldown prizes are pari-mutuel estimates. Actual = Pool ÷ Winners. Payout liability CAPPED at jackpot amount.*

> **🔒 PARI-MUTUEL PROTECTION:** During Quick Pick rolldown, all prizes transition from FIXED to PARI-MUTUEL. Total payout liability is EXACTLY $30,000-$50,000 (the jackpot), regardless of ticket volume or winner count.
...
|-----------|-------|----------|
| LP Reward Share | Configured on-chain (default configured by admin) | ADVANCED_FEATURES.md §7 |
...
> 💰 PASSIVE YIELD: The LP pool shares a portion of the fees collected on-chain. The reward share is configured on-chain by the program admin.
...
📚 **6 Documents** | 🎰 **2 On-Chain Programs (38+ instructions)**

> **🔒 CORE PROTECTION:** All prizes START as FIXED amounts, then TRANSITION to PARI-MUTUEL during rolldown events. Payout liability is ALWAYS CAPPED while players enjoy +EV windows.
