...
> **🔒 PRIZE TRANSITION SYSTEM:** All prizes START as FIXED amounts during normal mode, then TRANSITION to PARI-MUTUEL (shared pool) during rolldown events and high-volume draws. This hybrid system ensures **payout liability is ALWAYS CAPPED** while maintaining attractive +EV windows for players.

### Dynamic House Fee

A house fee is collected on-chain on each draw. The fee schedule scales with the jackpot level and is configured on-chain by the program.
...
- Whether 500k or 2M tickets are sold, the payout is the same
...
| **House Fee** | Configured on-chain (dynamic) |
...
> **🔒 PARI-MUTUEL PROTECTION:** During Quick Pick rolldown, prizes transition to pari-mutuel. Total payout liability is CAPPED at exactly $30,000-$50,000 (the jackpot), regardless of ticket volume or winner count.
...
### 🏆 Advanced Features
- **Dynamic House Fees**: Fees scale with the jackpot level (configured on-chain)
...
- ✅ **Fixed → Pari-Mutuel Prize Transition** (ALL prizes start fixed, transition to pari-mutuel during rolldown to cap payout liability)
...
- ✅ **Dynamic House Fee** (scales with jackpot level)
