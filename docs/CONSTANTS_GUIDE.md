...
| `HOUSE_FEE_USDC_SEED` | `b"house_fee_usdc"` | House fee USDC token account (collects the on-chain house fee from ticket sales) |
...
A house fee is collected on-chain on each draw. The fee rate scales with the jackpot level and is configured on-chain by the program. The fee is deducted from each ticket before the prize pool allocation described in [§5](#5-prize-allocation-system).

The fee tiers are defined by the program and read from on-chain config at runtime; they are intentionally not enumerated here. See the program source / IDL for the current values.
...
| Constant | Value | Description | Allocation (of `prize_pool_transfer`) |
|----------|-------|-------------|------------|
| `JACKPOT_ALLOCATION_BPS` | `5,560` BPS (55.6%) | Jackpot allocation | 55.6% |
| `FIXED_PRIZE_ALLOCATION_BPS` | `3,940` BPS (39.4%) | Fixed prize allocation | 39.4% |
| `RESERVE_ALLOCATION_BPS` | `300` BPS (3%) | Reserve fund allocation | remainder ≈ 5% |
| `INSURANCE_ALLOCATION_BPS` | `200` BPS (2%) | Insurance pool allocation | 2% (of post-fee pool) |

> **⚠️ Effective split** — `JACKPOT` and `FIXED` are applied to
> `prize_pool_transfer` (post-fee, post-insurance pool), and `reserve`
> captures the remainder, so the BPS do NOT sum to 100% (5,560 + 3,940 = 9,500).
> The effective shares of the **post-fee pool** are: jackpot ≈ 54.5%,
> fixed ≈ 38.6%, reserve ≈ 4.9%, insurance 2.0%. This is intentional (the
> remainder keeps the accounting exactly lossless — see
> `buy_ticket.rs::test_allocation_no_accounting_leak`); the documented 3%/2%
> reserve/insurance figures refer to reserve's explicit share plus the
> allocation gap captured as dust.
...
let after_house_fee = ticket_price - house_fee;

// 2% of the post-fee pool goes to the insurance pool token account.
let insurance_contribution = after_house_fee * INSURANCE_ALLOCATION_BPS as u64 / 10000;

// Everything else lands in the prize pool token account; the internal
// split below is accounting only (the cash is all in prize_pool_usdc).
let prize_pool_transfer = after_house_fee - insurance_contribution;

let jackpot_contribution = prize_pool_transfer * JACKPOT_ALLOCATION_BPS as u64 / 10000;
let fixed_prize_contribution = prize_pool_transfer * FIXED_PRIZE_ALLOCATION_BPS as u64 / 10000;
// Reserve = remainder (explicit 3% + allocation gap), keeping the sum exact.
let reserve_contribution = prize_pool_transfer - jackpot_contribution - fixed_prize_contribution;
```
...
> - **Total payout = EXACTLY the jackpot amount** (capped payout liability)
> - **Prize per winner = Pool Share × Jackpot ÷ Winner Count**
> - Payout liability is mathematically CAPPED regardless of volume
...
```rust
// PARI-MUTUEL: Prize = Pool ÷ Winners (payout liability CAPPED at jackpot)
let jackpot = 1_750_000_000_000; // $1.75M — this is the TOTAL payout liability
...
// Payout liability is CAPPED regardless of ticket volume or winner count!
```
...
> **🔒 PRIZE TRANSITION:** Quick Pick Express uses the same Fixed → Pari-Mutuel prize transition system as the main lottery. Normal mode prizes are FIXED; rolldown prizes are PARI-MUTUEL (payout liability CAPPED at jackpot amount).
...
The Quick Pick house fee scales with the jackpot level and is configured on-chain by the program. The fee tiers are read from on-chain config at runtime; see the program source / IDL for the current values.
...
> **🔒 PRIZE MODE: PARI-MUTUEL** — During rolldown, prizes are calculated as Pool ÷ Winners. Payout liability is CAPPED at exactly the jackpot amount ($30,000-$50,000).
...
> **🔒 PARI-MUTUEL PROTECTION:** During rolldown, the total payout is EXACTLY $30,000 (the jackpot) — regardless of whether 5,000 or 50,000 tickets are sold. The pari-mutuel system absorbs all volume risk.
...
pub fn calculate_quick_pick_fee_bps(jackpot_balance: u64, is_rolldown_pending: bool) -> u16 {
    // Fee is selected from the on-chain Quick Pick config (jackpot-level tiers).
    // See the program source / IDL for the current values.
    read_quick_pick_fee_from_config(jackpot_balance, is_rolldown_pending)
}
```
...
> **🔒 PARI-MUTUEL PROTECTION:** Both games use the Fixed → Pari-Mutuel prize transition system. During rolldown events:
> - **Main Lottery:** Payout liability CAPPED at $1,750,000-$2,250,000 (the jackpot)
> - **Quick Pick:** Payout liability CAPPED at $30,000-$50,000 (the jackpot)
>
> Regardless of ticket volume or winner count, the total payout is EXACTLY the jackpot amount during rolldown. The pari-mutuel system absorbs all volume risk while preserving player +EV.
