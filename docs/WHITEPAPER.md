...
Traditional lotteries operate on a simple principle: the house always wins. In a typical lottery, the majority of ticket value is retained by the house, and players face overwhelming negative expected value on every ticket purchased. While jackpot dreams attract players, the mathematical reality ensures consistent losses over time.
...
MazelProtocol proposes an alternative model that aligns incentives between the protocol and sophisticated players:
...
| **Sustainability** | Configurable on-chain house fee |
| **Accessibility** | $2.50 ticket price on low-fee Solana |
| **Favorable Distribution Events** | Rolldown mechanism creates +EV windows |

| **Payout Protection** | Fixed→Pari-Mutuel prize transition caps total payout |
...
>
> This hybrid system ensures **payout liability is always capped** while maintaining attractive +EV windows for players. During rolldown, the total payout is exactly the jackpot amount—no more, no less—regardless of how many tickets are sold or how many winners there are.
...
| Single authority risk | Multi-sig authority recommended; all state verifiable on-chain |
...
> **⚠️ PRIZE MODE: FIXED** — During normal operation, prizes are predetermined fixed amounts. This provides predictable player value but creates variable payout liability based on winner count.
...
**Pari-Mutuel Transition Trigger:** If (Winner Count × Fixed Prize) > Prize Pool Allocation, prizes automatically convert to pari-mutuel to cap the total payout.
...
> 1. **Payout liability is CAPPED** at exactly the jackpot amount $J$
...
**🔒 PARI-MUTUEL PROTECTION:** Total payout = $0.25J + 0.35J + 0.40J = J$ (exactly the jackpot). Payout liability is mathematically capped regardless of volume.
...
**Key Insight:** Higher volume REDUCES per-winner prizes but NEVER increases the total payout. The pari-mutuel system scales automatically to protect the protocol.
...
**🔒 PARI-MUTUEL PROTECTION:** Regardless of whether 500,000 or 2,000,000 tickets are sold during rolldown, the total payout is exactly $J$. The pari-mutuel system absorbs all volume risk.
...
>
> All prizes START as FIXED amounts during normal mode, then TRANSITION to PARI-MUTUEL (shared pool) during rolldown events and high-volume draws. This hybrid system ensures the total payout is ALWAYS CAPPED while maintaining attractive +EV windows for players.

### 4.1 Fund Flow Architecture

A house fee is collected on-chain on each ticket purchase; the remaining ticket value flows to the prize pool. The prize pool is allocated as follows (percentages of the prize pool):

| Component | Share | Role |
|-----------|-------|------|
| **Jackpot** | 55.6% | Growing prize pool |
| **Fixed Prizes** | 39.4% | Immediate (FIXED MODE) |
| **Reserve Fund** | 3% | Buffer |
| **Insurance Pool** | 2% | Solvency protection |

When a rolldown occurs, the prize pool transitions to PARI-MUTUEL distribution.

### 4.1.1 Prize Mode Transition System

| Mode | When Active | Prize Calculation | Payout Liability |
|------|-------------|-------------------|-------------------|
...
**Why This Matters:** During a rolldown with 1M+ tickets, fixed prizes could exceed pool capacity. Pari-mutuel ensures total payout = jackpot amount (CAPPED), regardless of volume.

### 4.2 Two-Week Cycle Economics

The protocol operates in cycles: a normal period in which prizes are FIXED amounts and the jackpot grows, followed by a rolldown period in which prizes transition to PARI-MUTUEL and the accumulated jackpot is distributed.

Prize payouts (both fixed and pari-mutuel) are funded entirely from the prize pool, which is self-sustaining from ticket sales. During a rolldown, the total payout is capped at the jackpot amount regardless of ticket volume or winner count.

### 4.3 Sensitivity Analysis

With pari-mutuel, the total payout during a rolldown is capped at the jackpot amount independent of volume, so the payout scales predictably. In fixed-prize mode, payout liability grows with the winner count, which is why the Fixed → Pari-Mutuel transition is applied during high-volume draws.

### 4.3.1 Prize Mode Comparison: Fixed vs Pari-Mutuel

| Scenario | Fixed Prizes (Hypothetical) | Pari-Mutuel (Actual) | Payout Impact |
|----------|----------------------------|---------------------|------------------|
...
| 2M tickets rolldown | ~$1.58M variable | $1,750,000 capped | Payout stays capped |

**🔒 KEY PROTECTION:** With fixed prizes, higher volume = higher payout liability (unbounded). With pari-mutuel, higher volume = SAME payout (capped at jackpot). This is why the Fixed→Pari-Mutuel transition is applied for protocol sustainability.
...
- **Impact:** The jackpot is paid and the cycle resets

**Scenario B: Multiple Jackpot Winners**

If multiple Match-6 winners in one draw:
- Jackpot split among all winners
...
- **Impact:** The total payout stays capped at the jackpot
...
The Jackpot LP Pool replaces centralized jackpot seeding with community-provided liquidity. LPs deposit USDC to help fund jackpot seeds after rolldowns and wins, and earn a proportional share of the fees collected on-chain. This shifts the protocol's capital structure from authority-funded to community-funded while creating a yield-bearing asset for depositors.

#### Rewards

A house fee is collected on-chain from each ticket purchase. A configured share of that fee accrues to the LP pool, and the remainder of each ticket flows to the prize pool. The prize pool is allocated as described in §4.1. See the program source / IDL for the configured reward share.

#### Risk Analysis

| Risk | Severity | Mitigation |
|------|----------|------------|
| LP pool drained by consecutive rolldowns | Medium | Reserve fund (3%) is the fallback; seed can be reduced via config |
| Low ticket volume | Medium | LP reward % is adjustable |
| Whale LP exit causes seed shortage | Low | Withdrawal gate blocks exits during active draw cycles |
| LP rewards < seed cost over time | Low | Configurable seed_amount and lp_reward_bps allow tuning |
...
### 5.3 Protocol-Player Dynamics

**The Core Tension:**

The protocol wants: high volume, predictable cycles, sustainable operation
...
- The protocol benefits from increased rolldown volume (higher ticket sales)
- Players profit from the +EV window (if volume stays reasonable)
...
During rolldown, the total payout is capped by pari-mutuel at the jackpot, so the protocol's outcome scales with volume while players' +EV is preserved when volume is below the break-even threshold described in §3.4.
...
**Budget Balance:** Configurable on-chain fees cover operational costs
...
fn trigger_rolldown_internal(
    ...
    // Payout liability is CAPPED at exactly the jackpot amount
...
The Quick Pick house fee scales with the jackpot level and is configured on-chain by the program.

**Prize Pool Allocation (after house fee):**
...
├── House Fee: Collected on-chain (configured by the program)
└── Prize Pool:
...
1. **Fund Segregation**: Player funds are never commingled with protocol treasury funds
...
3. **For the Protocol:** Sustainable operation with transparent on-chain fees
