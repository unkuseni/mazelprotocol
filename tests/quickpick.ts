...
describe("quickpick", () => {
  ...
  const mainProgram = anchor.workspace.mazelprotocol as any as Program;
  const qpProgram = anchor.workspace.quickpick as any as Program;

  const mainProgramId = mainProgram.programId;
  const qpProgramId = qpProgram.programId;

  const typedMainProgram = mainProgram as Program<any>;
  const typedQpProgram = qpProgram as Program<any>;
  ...
  before(async () => {
    ...
    for (let i = 0; i < ticketsToBuy; i++) {
      const mainState = await typedMainProgram.account.lotteryState.fetch(
