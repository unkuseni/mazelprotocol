...
export interface MainLotteryProgram {
	...
	>;
	/**
	 * Anchor's fluent transaction builder (`.methods.<ix>(...).accounts(...)`).
	 * Loosely typed to the IDL — callers chain `.methods.X().accounts().instruction()`.
	 */
...
export async function createMainLotteryProgram(
	...
	return program as unknown as MainLotteryProgram;
}
...
export async function createQuickPickProgram(
	...
	return program as unknown as QuickPickProgram;
}
