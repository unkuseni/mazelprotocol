...
// Wallet
import { useWallet, useConnectedWallet } from "./wallet";
...
export interface MazelProtocolState {
  ...
  /** Read-only main lottery program client */
  mainProgram: MainLotteryProgram;
  /** Read-only Quick Pick program client */
  qpProgram: QuickPickProgram;
...
export interface MazelProtocolActions {
  ...
  buyQuickPickTicketsBulk: (tickets: BuyQuickPickTicketParams[]) => Promise<string>;
  ...
  const actions: MazelProtocolActions = useMemo(() => {
    ...
    return {
      ...
      buyQuickPickTicketsBulk: async (tickets: BuyQuickPickTicketParams[]) => {
        ...
        const userStats = deriveUserStatsPDA(wallet.publicKey!);
        const playerUsdc = getUserUsdcAccount();
        return buyQuickPickTicketsBulk(provider, tickets, userStats, playerUsdc);
      },
