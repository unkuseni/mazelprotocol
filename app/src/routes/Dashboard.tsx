...
import { WinningNumbers, LotteryBallRow } from "@/components/LotteryBalls";
...
function StatCard({
  ...
  return (
    <div className="p-4 rounded-2xl bg-card/50 border border-border/50">
      ...
      <div className={cn("text-2xl font-black tracking-tight", colors.valueColor)}>
        {value}
      </div>
      {sub && (
    ...
    >
      <div className="flex items-center justify-between mb-3">
        ...
        {wasRolldown && (
          <div className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-emerald-500/15 border border-emerald-500/20">
            ...
            <span className="text-[9px] font-bold text-emerald-400 uppercase">Rolldown</span>
          </div>
        )}
      </div>
      ...
      <div className="grid grid-cols-3 gap-2 mt-4 text-center">
        <div>
          ...
          <p className="text-xs font-bold text-foreground">{totalTickets.toLocaleString()}</p>
        </div>
        <div>
...
function ActiveTicketsPanel() {
  const { tickets, unclaimedTickets, unclaimedPrizeTotal, loading } = useTickets();
        ...
        <div className="mb-4 p-3 rounded-xl bg-gold/5 border border-gold/10">
          <div className="flex items-center justify-between">
            <span className="text-xs font-semibold text-gold">
              {unclaimedTickets.length} unclaimed prize{unclaimedTickets.length > 1 ? "s" : ""}
            </span>
            ...
            >
              <div className="flex items-center gap-3">
                <div className={cn(
                  ...
                  ticket.matchCount >= 3 ? "bg-gold" : ticket.matchCount === 2 ? "bg-emerald" : "bg-muted-foreground/30",
                )} />
                <div>
                  <LotteryBallRow
...
export default function DashboardPage() {
  ...
  const playerStats = useMemo(() => {
    ...
    const totalSpent = tickets.length * 2.50;
    const totalWon = tickets.reduce((sum, t) => sum + Number(t.prize) / 1_000_000, 0);
    const netProfit = totalWon - totalSpent;
    const winningTickets = tickets.filter((t) => t.matchCount >= 2).length;
    const winRate = tickets.length > 0 ? (winningTickets / tickets.length) * 100 : 0;
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  {draws.slice(0, 4).map((draw) => (
                    <RecentDrawCard
                      key={draw.drawId}
                      drawId={draw.drawId}
                      date={new Date(Number(draw.timestamp) * 1000).toLocaleDateString("en-US", {
                        month: "short",
                        day: "numeric",
                        hour: "2-digit",
                        minute: "2-digit",
                      ...
                      jackpot={formatCurrency(Number(draw.jackpotAtDraw) / 1_000_000)}
