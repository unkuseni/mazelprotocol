...
// ---------------------------------------------------------------------------
// Signed chat actions (security review H6)
//
// The chat API now requires proof of wallet ownership for every mutating
// action (send / react / pin). We sign the canonical message
//
//     "mazelprotocol:chat:{context}:{timestamp}:{nonce}"
//
// where `context` is the syndicate id (for messages) or message id (for
// reacts/pins). The server verifies the Ed25519 signature, checks freshness,
// and rejects replayed nonces — so `sender` can no longer be spoofed.
// ---------------------------------------------------------------------------

const CHAT_SIGN_PREFIX = "mazelprotocol:chat:";

type SignMessageFn = (message: Uint8Array | string) => Promise<Uint8Array>;

async function signChatPayload(
	...
	const message = `${CHAT_SIGN_PREFIX}${context}:${timestamp}:${nonce}`;
	const sigBytes = await signMessage(message);
	const binary = Array.from(sigBytes, (b) => String.fromCharCode(b)).join("");
	return { signature: btoa(binary), timestamp, nonce };
}
...
export function useChat({
	...
	// SECURITY (review H6): wallet message-signing capability for authenticated
	// chat actions (send / react / pin).
	...
	const sendMessage = useCallback(
		async (
			...
			try {
				const auth = await signChatPayload(signMessage, syndicateId);
		...
		[syndicateId, sender, signMessage],
	);
	...
	const reactToMessage = useCallback(
		async (messageId: string, emoji: string, action: "add" | "remove") => {
			...
			try {
				const auth = await signChatPayload(signMessage, messageId);
				await fetch(reactUrl(messageId), {
					method: "POST",
					headers: { "Content-Type": "application/json" },
					body: JSON.stringify({ emoji, action, sender, ...auth }),
				});
			} catch {
		...
		[sender, signMessage, fetchMessages],
	);
		...
		async (messageId: string, pinned: boolean) => {
			...
			try {
				...
				await fetch(pinUrl(messageId), {
					method: "POST",
					headers: { "Content-Type": "application/json" },
					body: JSON.stringify({ pinned, sender, ...auth }),
				});
			} catch {
				fetchMessages();
			}
