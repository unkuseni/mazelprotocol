...
function mapDrawResult(
	...
	return {
		...
		jackpotAtDraw: toBigInt(
			g("jackpot_at_draw", "jackpotAtDraw") ??
				g("jackpot_balance", "jackpotBalance"),
		),
...
export function useDraws(): UseDrawsReturn {
	...
	const currentDrawId = useMemo(() => {
		...
		return toNumber(
			(lotteryState as Record<string, unknown>).current_draw_id ??
				(lotteryState as Record<string, unknown>).currentDrawId,
		);
	}, [lotteryState]);
	...
	const draws = useMemo(() => {
		return drawQueries
			...
			.filter(
				(d): d is DrawResultData => d !== null && d.winningNumbers.length > 0,
			);
	}, [drawQueries]);
