    ...
    <div className={cn("p-6 rounded-2xl bg-card/50 border border-border/50", className)}>
      ...
      {rolldownActive && (
        <div className="mb-6 p-4 rounded-xl bg-muted/30 border border-border/30">
          ...
          <div className="flex justify-between mt-1.5">
            <span className="text-[9px] text-muted-foreground">50K (low volume)</span>
            <span className="text-[9px] text-muted-foreground">1.5M (high volume)</span>
          </div>
          <p className="text-[10px] text-muted-foreground/60 mt-2">
            More tickets = more winners per tier = lower per-winner prizes.<br />
      ...
      {rolldownActive && (
        <div className="mt-4 flex items-start gap-2 p-3 rounded-lg bg-foreground/3 border border-foreground/5">
          ...
          <p className="text-[10px] text-muted-foreground leading-relaxed">
            Prizes are <span className="font-semibold text-foreground">pari-mutuel</span> during
            rolldown — per-winner amounts decrease as more tickets are sold. This
            protects the protocol from unbounded liability while maintaining fair
            distribution. The more popular the draw, the lower your per-ticket edge.
          </p>
        </div>
      )}
      ...
      {!rolldownActive && (
        <div className="mt-4 flex items-start gap-2 p-3 rounded-lg bg-amber-500/5 border border-amber-500/10">
          ...
          <p className="text-[10px] text-muted-foreground leading-relaxed">
            During normal mode, the house has an edge. But once the jackpot reaches{" "}
            <span className="font-semibold text-foreground">$1.75M</span> and rolldown
            activates, prizes become pari-mutuel and the edge flips in favor of players.
            This is the +EV window that makes MazelProtocol unique.
          </p>
        </div>
      )}
    </div>
...
export function EVBadge({
  ...
  if (loading) {
    return (
      <div className={cn("inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-muted/50 animate-pulse", className)}>
  ...
  if (!data) {
    return (
      <div className={cn("inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-foreground/5 border border-foreground/10", className)}>
