	...
	return (
        ...
        `}
			>
				...
				<div className="absolute inset-0 bg-linear-to-br from-surface-1/90 via-surface-0/70 to-surface-2/90 backdrop-blur-sm border border-gold-500/10 rounded-2xl" />
				...
				<div className="relative z-10 flex flex-col items-center text-center">
					...
					<div className="flex items-center gap-2 mb-2">
						<Trophy size={config.iconSize} className="text-gold-300" />
						<span
							className={`${config.labelClass} font-semibold text-gold-300/80 uppercase tracking-wider`}
						>
							{label}
						</span>
					</div>
					...
					{showRolldownStatus && (
						<div className="mt-3">
							{rolldownActive ? (
								<div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-emerald-400/15 border border-emerald-500/30">
									<div className="w-1.5 h-1.5 rounded-full bg-emerald-400 shadow-[0_0_10px_rgba(0,255,159,0.35)] animate-pulse" />
					...
					{softCap > 0 && size !== "sm" && (
						<div className="w-full max-w-xs mt-4">
							<div className="flex items-center justify-between text-[10px] text-muted-foreground mb-1.5">
								...
								<span className="text-gold-300/70 font-medium">
									{progress.toFixed(1)}%
								</span>
							</div>
...
export function JackpotBadge({ amount, className = "" }: JackpotBadgeProps) {
	return (
		<div
			className={`inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-gold-400/10 border border-gold-500/20 ${className}`}
		>
