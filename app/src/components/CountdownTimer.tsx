...
function TimeUnitBox({ value, label, size, isUrgent }: TimeUnitBoxProps) {
	...
	return (
		<div className="flex flex-col items-center">
          ...
          ${
						isUrgent
							? "bg-linear-to-b from-red-500/15 to-red-600/10 border border-red-500/40 shadow-[0_0_16px_rgba(255,51,85,0.25)]"
							: "bg-surface-2/60 border border-cyan-500/25 shadow-[0_0_14px_rgba(0,229,255,0.08)]"
					}
        `}
			>
				<span
            ...
            ${isUrgent ? "text-red-400 [text-shadow:0_0_10px_rgba(255,51,85,0.6)]" : "text-cyan-300 [text-shadow:0_0_10px_rgba(0,229,255,0.55)]"}
          ...
          `}
			...
			<span
				className={`
          ${config.labelClass}
          mt-1.5 uppercase tracking-wider font-medium font-mono
          ${isUrgent ? "text-red-400/70" : "text-muted-foreground"}
        `}
			>
				{label}
			</span>
		...
		return (
			<div className={`flex flex-col items-center ${className}`}>
				<div className="flex items-center gap-2 mb-3">
					...
					/>
					<span
						className={`${config.headerClass} font-semibold text-emerald-300 uppercase tracking-wider`}
					>
						Drawing Now
					</span>
				</div>
				<div className="flex items-center gap-2 px-4 py-2 rounded-full bg-emerald-400/10 border border-emerald-500/20">
					<div className="w-2 h-2 rounded-full bg-emerald-400 shadow-[0_0_8px_rgba(0,255,159,0.9)] animate-ping" />
...
export function InlineCountdown({
	...
	if (timeLeft.total <= 0) {
		return (
			<span
				className={`inline-flex items-center gap-1.5 text-emerald-300 ${className}`}
			>
				<div className="w-1.5 h-1.5 rounded-full bg-emerald-400 shadow-[0_0_8px_rgba(0,255,159,0.9)] animate-ping" />
...
interface QuickPickCountdownProps {
	...
	className?: string;
	/**
	 * On-chain next draw timestamp (Unix ms) from QuickPickState.nextDrawTimestamp.
	 * When provided, it is used as the countdown target; otherwise the next
	 * 4-hour UTC boundary is computed from the client clock as a fallback.
	 */
	nextDrawTimestamp?: number;
}

/** Compute the next 4-hour UTC draw boundary from the client clock (fallback). */
function getNext4hBoundary(): number {
	const now = new Date();
...
export function QuickPickCountdown({
	...
	const [target, setTarget] = useState(() => getNext4hBoundary());

	// Use the on-chain next draw timestamp when it is available and still in
	// the future, falling back to the client-clock 4-hour boundary when it is
	// not (e.g. before the state arrives, or right after a draw completes and
	// the next on-chain timestamp has not been fetched yet).
	const effectiveTarget =
		nextDrawTimestamp && nextDrawTimestamp > Date.now()
			? nextDrawTimestamp
			: target;

	const [timeLeft, setTimeLeft] = useState<TimeLeft>(
		getTimeLeft(nextDrawTimestamp ?? target),
	);

	useEffect(() => {
		setTimeLeft(getTimeLeft(effectiveTarget));
	}, [effectiveTarget]);

	useEffect(() => {
		const interval = setInterval(() => {
			const updated = getTimeLeft(effectiveTarget);
			setTimeLeft(updated);

			// If completed, calculate the next 4-hour window
			if (updated.total <= 0) {
				setTarget(getNext4hBoundary());
			}
		}, 1000);
	...
	return (
		<div className={`flex items-center gap-1.5 ${className}`}>
			<Zap
				size={size === "sm" ? 12 : 14}
				className={isUrgent ? "text-red-400" : "text-cyan-300"}
			/>
			<span
